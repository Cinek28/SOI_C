...
PRIVATE int FIT = FIRST;	/* current allocation algorithm */
...
{
  register struct hole *iter;	/* iterator */
  phys_clicks buffer[NR_HOLES * 2 + 1], count, i = 0;
  count = mm_in.m1_i1 / (sizeof(phys_clicks) * 2); /* no of entries to fill */
  for(iter = hole_head; i < count && iter != NIL_HOLE
   && iter->h_base < swap_base; iter = iter->h_next) {
	buffer[i++] = iter->h_len;
	buffer[i++] = iter->h_base;
  }
  buffer[i] = 0; /* last entry */
  sys_copy(MM_PROC_NR, D, (phys_clicks) buffer, who, D,
	  (phys_clicks) mm_in.m1_p1, (phys_clicks) mm_in.m1_i1); /* filling */
  return i / 2;
}
 ...
 */
  ...
  register struct hole *hp, *prev_ptr, *hp_temp, *prev_ptr_temp;
 ...
 case WORST:
  do {
  	prev_ptr_temp = NIL_HOLE;	/* prev_ptr_temp - worst one's previos */
	hp_temp = hp = hole_head;	/* hp_temp - worst one */
	while(hp != NIL_HOLE && hp->h_base < swap_base) {
		if(hp->h_len > hp_temp->h_len) {
			hp_temp = hp;
			prev_ptr_temp = prev_ptr;
		}
		prev_ptr = hp;
		hp = hp->h_next;
	}
	if(hp_temp->h_len >= clicks) {
		/* We have the worst fit.  Use it. */
		old_base = hp_temp->h_base;
		hp_temp->h_base += clicks;
		hp_temp->h_len -= clicks;

		/* Delete the hole if used up completely. */
		if(!hp_temp->h_len)
			del_slot(prev_ptr_temp, hp_temp);      	

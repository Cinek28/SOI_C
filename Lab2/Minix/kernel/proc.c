 ...
 */
  ...
  register struct proc* pr = rdy_head[USER_Q];
  struct proc* temp = pr;
  ...
  if(quants == 0){
	if(rdy_head[USER_Q]->counts == groupA){
		searching = groupB;
	}
	else if(rdy_head[USER_Q]->counts == groupB){
		searching = groupC;
	}
	else if(rdy_head[USER_Q]->counts == groupC){
		searching = groupA;
	}
	while(pr->p_nextready != NIL_PROC){
		temp = pr; 
		pr = pr->p_nextready;
		if(pr->counts == searching){
			rdy_tail[USER_Q]->p_nextready = rdy_head[USER_Q];
			rdy_tail[USER_Q] = rdy_head[USER_Q];
			if(temp!=rdy_head[USER_Q])
				temp->p_nextready = pr->p_nextready;
			temp = rdy_head[USER_Q]->p_nextready;
			rdy_head[USER_Q] = pr;
			if(temp!=rdy_head[USER_Q])
				rdy_head[USER_Q]->p_nextready = temp;
